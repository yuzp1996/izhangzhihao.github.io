...
class Solution:
    def isSymmetrical(self, pRoot):
        ...
        def is_same(a,b):    # 猪蹄函数，
            if not a and not b:  #比较完成，也就是没有出现差错
                return True
            if a and b and a.val == b.val:  #比较成功
                return is_same(a.right,b.left) and is_same(a.left,b.right)
            return False
        ...
        return is_same(pRoot.left,pRoot.right)  #入口 将根的左右节点放入。
```
